...
def sm_hist(data, delta=5, n_bin=None, range_=None, density=False):
	dataMin = np.floor(data.min())
	dataMax = np.ceil(data.max())
	n_bin = np.ceil(1.*(dataMax-dataMin) / delta)
	range_ = (dataMin, dataMin + n_bin * delta)
	counts, bin_edges = np.histogram(data, n_bin, range_, density = density)
...
class dataO(object):
	...
	def freqs(self, delta=None):
		...
		if self.delta == 0:
			...
			sys.exit(1)
		self.counts, self.bins = sm_hist(self.data, delta=self.delta, density=False)
	...
	def myPercentile(self, which):
		...
		return np.sort(self.data)[idx] # non dovrei sortare al contrario mettendo i più alti prima??
...
for infile in ["CODAchain1.txt", "CODAchain2.txt", "CODAchain3.txt"]:
	...
	histPageWidth = "20cm"
	histPageHeight = str(10*len(idxs))+"cm"

	histPage = document.Root.Add('page', width=histPageWidth, height=histPageHeight)
	contourPage = document.Root.Add('page', width="20cm", height="10cm")
	...
	contourGridRows = 1
	contourGridColumns = 2

	contourGrid = contourPage.Add('grid', autoadd=False, rows = contourGridRows, columns = contourGridColumns,
								bottomMargin='0cm',
								leftMargin='0cm'
								)
	...
	for key in data.keys():
		...
		dataOut[data[key].name + ":" + "percentile:" + str(50)] = [data[key].percentile(50)]
		dataOut[data[key].name + ":" + "percentile:" + str(68)] = [data[key].percentile(68)]
		dataOut[data[key].name + ":" + "percentile:" + str(90)] = [data[key].percentile(90)]
		dataOut[data[key].name + ":" + "percentile:" + str(95)] = [data[key].percentile(95)]
		#print percentile, " percentile", data[key].percentile(percentile), data[key].myPercentile(percentile)
		dataOut[data[key].name + ":" + "bins"] = data[key].bins
		dataOut[data[key].name + ":" + "counts"] = data[key].counts
		
		print "Variable ", key, " mean ",  data[key].mean, " std ", data[key].std, " p50 ", data[key].percentile(50), " p68 ", data[key].percentile(68), " p90 ", data[key].percentile(90), " p95 ", data[key].percentile(95), 
	...
	if len(idxs) > 1:
		for combination in itertools.combinations(data.keys(), 2):
			...
			### Colormap
			graph = contourGrid.Add('graph', autoadd=False)
			xAxis = graph.Add('axis', name='x', label = "")
			yAxis = graph.Add('axis', name='y', label = "")
			xy = graph.Add('image')
						
			dataName = "hist2D"      
			document.SetData2D(dataName, hist2D)
			xy.data.val = dataName
